...
> **2026-09-19 权威等级澄清（只加限定，不改上文任何结论）**：上文"唯一产品定义"的 scope 是
> **产品定义这一层内唯一**——即 §1 是什么 / §2 不是什么 / §5 复用优先级 / §6 废止方向 / §11 仓库结构以本文件为准。
> 它**不高于** `CONSTITUTION.md`（该文件规定使命、绝对禁令与开发前五问，冲突时上位赢）；
> 它**不描述**当前线上真实状态（那归 `CURRENT_STATE.md`）；它也**不授权**任何开发。
> 其中 §10「V0.1 SCOPE」已完成并上线，属历史记录，不作为下一轮范围。
