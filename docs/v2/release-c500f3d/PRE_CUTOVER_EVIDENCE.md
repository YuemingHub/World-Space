...
令牌提取修好后，又跑了一遍 **不花钱**的 `--no-loop`（只验与搜索无关的一半）：

```text
PASS=23 FAIL=0 SKIPPED=3      本机账本 today 仍 15、month 仍 ¥0.211396 → 这遍额外花费 ¥0
✗ B 认到 u-guest  →  现在 ✓（B 的身份实测 {"user_id":"u-guest"}，A/B 是两个不同令牌）
跳过的 3 项显式记 SKIPPED，不记绿：脚本汇总在 SKIPPED>0 时只印
  LOCAL_SMOKE=PARTIAL 并写明"这一轮不能当验收"——半张体检表不许冒充全表
```

换上能用的搜索 key 后，跑**完整那一轮**（真实模型 + 真实搜索）：

```text
PASS=25 FAIL=1 SKIPPED=0
第一轮 HTTP=200（36s）  next_action="今晚天黑后到坏灯位置拍 1 张照片和 1 段 10 秒视频
  （画面含路灯杆编号或周边店招、路口等参照物），并在备忘录写清：具体位置、首次发现
  不亮的日期、每晚无照明的大致时段。"
  done_when="手机里存有至少 1 张照片和 1 段视频，且备忘录里有一行完整记录：
             具体位置 + 首次发现不亮的日期 + 每晚无照明的大致时段。"   mode=human
  meta.search_calls=1  → 真实搜索发生了，新 key 走通
第二轮 HTTP=200（104s，第 1 次撞上游 429、退避 40s 后第 2 次成功）
  receipt_ingested=true  receipt_status=stuck
  行动改为"…并顺手确认这盏灯在小区围墙内还是门外市政道路上" → 与第一轮不同，
  没有把原始意图当新问题重答
唯一那条 ✗ 又是我的脚本自己造成的假红：清 .r2ok 标记的那行放在了"写完标记之后、读标记之前"，
等于把刚写下的证据删掉再判。为此加了 --rejudge：对**同一批已抓到的响应字节**重判，
不再发任何请求 → PASS=2 FAIL=0 REJUDGE=PASS，额外花费 ¥0。
（重判前先卡三道闸门防"拿旧文件冒充本轮"：抓取时间 3 小时内、响应体不含 error、两轮都有 next_action）
```

本轮真实计费增量：本机账本 `today 15→23`、`month ¥0.211396→¥0.364926`（这轮 ¥0.153）；
公网账本不变（¥1.288846）。**本月真实累计 ¥1.65 / ¥20**。
...
**A（可证明）** 控制台删掉这两把、新建一把、只把新的单独写进 ws-key.txt ——
新指纹必然不同于已知两把，"没在聊天里出现过"就成了一件**证明**而不是相信的事；
**B（她指名）** 直接告我文件里第几把是今天新建的，由她承担这个判断。

下一步（Founder 在 Tavily 控制台做一次，之后我一轮收尾）：

```text
本轮实况：Founder 回"就这个"并重新附了同一个 ws-key.txt —— 但文件里仍然是**那两把不同的 key**
（指纹仍是 74f41d0013af 与 4002d0bdf8d6，没有变化），所以"这个"指哪一把仍然不唯一，
不能靠猜把一把可能已在会话里出现过的 key 定成生产 key。

为什么"再新建一把"是唯一可证明的路线：
  新 key 的指纹必然不同于已知这两把 → 它不可能是聊天里出现过的那把（这是**证明**，不是相信）；
  旧的两把在控制台删除后，我拿服务器已有的原文各发一次请求，全变 401 → 这才是
  OLD_TAVILY_KEY_REVOKED=yes 的物证。
