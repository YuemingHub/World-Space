...
→ hero（「你想做什么事？」+ 7 个目标按钮 + 1 个兜底宽按钮）
   └─ 7 个目标按钮 → 按需展开目标面板（写点东西 / 做个网页 / 查明一件事 / 整理资料·处理 PDF / 做一张海报 / 整理手机照片 / 提取图片文字）：
        ...
        查：DeepSeek 深度思考 + 联网搜索 + 文件拖进输入框上传（免费已核证）+ 30 秒核实清单（官方渠道确认钱/政策/法律）；副标题覆盖「翻译文件」
        资料/PDF：乱文字→豆包整理；录音转文字→通义听悟（免费 90 天、每天 2 小时，官方文档核证；要登录、90 天后收费如实标注）；PDF 文件→Stirling PDF（需电脑；手机替代路径如实标注）
        海报：准确信息→稿定设计模板改字；个人用→豆包 AI 生图（含复制模板句）
        照片：去重（iPhone/安卓原生+「合并不是清空相册」安心话）、备份（自带云/一刻相册）、印刷相册（照片书小程序）、修图（醒图）
