...
→ hero（「你想做什么事？」+ 副标题原则句"打开工具只是第一步，真正动手，看看现实里发生了什么" + 7 个目标按钮 + 1 个兜底宽按钮）
   └─ 7 个目标按钮 → 按需展开目标面板（写点东西 / 做个网页 / 查明一件事 / 整理资料·处理 PDF / 做一张海报 / 整理手机照片 / 提取图片文字）：
        每个面板按真实子目标给一个默认能力 + 第一步（可复制）+ 做到什么算开始 + 卡住怎么办 + 如实标注 + 现实结果（写/网页/查明三处明写；海报/照片/资料/OCR 随步骤自然可见）
        写：豆包模板句 + 改稿循环（初稿事实需自己过目；PPT 大纲/发言稿同一招；改到能直接发出去的文字为止）
        网页：豆包对话生成预览；进阶「发布上线」→ v0（免费能开始，用量大了要付费；发布后得到一个网址，别人手机点开就能看到）
        查：DeepSeek 深度思考 + 联网搜索 + 30 秒核实清单（官方渠道确认钱/政策/法律）；核实完能说清"哪几点已确认、哪几点还不确定"
