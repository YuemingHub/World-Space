...
→ hero（「你想做什么事？」+ 7 个目标按钮 + 1 个兜底宽按钮）
   └─ 7 个目标按钮 → 按需展开目标面板（写点东西 / 做个网页 / 查明一件事 / 整理资料·处理 PDF / 做一张海报 / 整理手机照片 / 提取图片文字）：
        ...
        写：豆包模板句 + 改稿循环（初稿事实需自己过目；PPT 大纲/发言稿同一招）
        网页：豆包对话生成预览；进阶「发布上线」→ v0（免费能开始，用量大了要付费）
        查：DeepSeek 深度思考 + 联网搜索 + 文件拖进输入框上传（免费已核证）+ 30 秒核实清单（官方渠道确认钱/政策/法律）
        资料/PDF：乱文字→豆包整理；PDF 文件→Stirling PDF（需电脑；手机替代路径如实标注）
...
→ 1 张阶段卡（手风琴）「三步上手：做成、改好、查证」：
    阶段引言（一段话，不占步骤）
    → 3 个编号步骤（手风琴）：① 豆包做一件小事 ② 让它改成你满意的样子 ③ 查一件事并核实
    → 阶段检查点（把用户交回首页目标按钮）
