...
> **2026-09-19 scope 澄清（只加限定，下文一字不改）**：标题里的"当前线上"已经**不再成立**。
> 自 2026-09-16 00:15 起，公网 `ymai.fun` 实际跑的是 **V0.1（`web/v2/`，`v2` @ `e8284c4`，带访问门）**，
> 首屏是自由输入框「你现在想做成什么？」，不是本文件描述的 7 目标按钮 + 三步兜底。
> 本文件从"当前设计真源"降级为 **V1 静态站的设计真源**，用于追溯 V1 为什么长成那样；
> 它**不约束** V0.2，也**不要**被引用为"线上现在是什么样"。线上事实看仓库根目录 `CURRENT_STATE.md`。
