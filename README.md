...
| 你想做的事 | 会发生什么 |
|---|---|
| ✍️ 写点东西 / 🛠️ 做个网页 / 🔍 查明一件事 / 🗂️ 整理资料·处理 PDF | 按需展开面板：一个默认工具 + 可复制的第一步 + 如实限制（做网页的进阶选项：v0 发布上线；查文件直接拖进输入框上传；资料面板含录音转文字通义听悟） |
| 🖼️ 做一张海报 | 按需展开：要放准确信息→稿定设计模板改字；个人用→豆包 AI 直接画 |
| 📷 整理手机照片 | 去重（手机自带）、备份、视频相册（剪映一键成片）、印刷相册、修图，每件一个默认做法 |
